...
/* An (273,191) Low-Density Parity-Check code (LDPC) is simulated      */
...
#define MAX_NODES    273   				// Maximum of number of code/check nodes
#define MAX_CHK      17   					// Maximum number of checks per code bit
#define MAX_BIT      17   					// Maximum number of code bits per check
#define num_packet 	40000					// number of packets simulated
...
const int n = 273;               		// Code word length of a linear block code
const int k = 191;                		// Information bits of a linear block code
int n_k;                          		// Redundancy (Parity check bits)
float rate;                      		// code rate
int M, N;                          		// Size of parity-check matrix (Row,Column) = (M,N)
double q[273][10][2], snr_rms;
...
int main(void)
{
   ...
   start = time(NULL);
   printf("BER Performance of (273,191) LDPC Code in AWGN Channel\n");
   printf("Dimension of Parity Check matrix: (273,273)\n");
   printf("Column Weight = 17, Row Weight = 17\n");
   printf("Number of bits of simulation = %d\n\n", n*num_packet);
   printf("This program is running. Don't close, please!\n\n");

   records = fopen("Records_273_191_BP_awgn.log", "a");
   fprintf(records, "BER Performance of Galager (273,191) LDPC Code in AWGN Channel\n");
   fprintf(records, "Dimension of Parity Check matrix: (273,273)\n");
   fprintf(records, "Column Weight = 17, Row Weight = 17\n");
...
/*   out_prob = new double**[MAX_NODES];			// estimated bit probability
   ...
   err_rate = new double[Iteration];
	...
	// Define Parity Check Matrix
  	parity_check = fopen("DSC_273_17_17.log", "r");
   ...
   #ifdef PRINT_MATRIX
  	printf("%d %d\n", N, M);
  	printf("%d %d\n", max_weight_N, max_weight_M);
  	for (i=0; i<M; i++)
      printf("%4d", num_check_N[i]);
  	printf("\n");
  	for (i=0; i<N; i++)
    	printf("%4d", num_check_M[i]);
  	printf("\n");
   getch();
  	for (i=0; i<M; i++)
  	{
    	for (j=0; j<num_check_N[i]; j++)
      	printf("%4d", check_set_N[i][j]);
    	printf("\n");
  	}
   getch();
  	for (i=0; i<N; i++)
...
/*===========================================*/
   ber=fopen("ber_LDPC_273_191.log", "w");
   for(snr=0; snr<=5; snr+=0.5)
   {
...
// Based on Pearl's book and MacKay's paper
	...
	for (i=0; i<N; i++)		// for every bit node
   {
  	...
  	for (i=0; i<M; i++)		// for every check node
    	for (j=0; j<num_check_N[i]; j++)		// 這一步驟好像沒意義???
      {
      	qm[i][j][0] = 0.0;
      	qm[i][j][1] = 0.0;
      }
   ...
   for(iter=0; iter<Iteration; iter++)		// iterations of massage passing
   {
      	...
      	{
      		delta[i][check_set_N[i][j]-1] = 1.0;	// initial value for 累乘
      	...
      	for (j=0; j<num_check_M[i]; j++)
      	{
		   	qm[check_set_M[i][j]-1][i][0] = 1.0;	// initial value for 累乘
